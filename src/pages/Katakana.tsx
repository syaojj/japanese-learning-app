...
import { katakana } from '../data/japaneseData'
import './Alphabet.css'
...
function Katakana() {
  ...
  const groupedChars = katakana.reduce((acc, char) => {
    if (!acc[char.row]) {
      acc[char.row] = []
    }
    acc[char.row].push(char)
    return acc
  }, {} as Record<string, typeof katakana>)
        ...
        <div className="character-detail">
          <div className="detail-content">
            <div className="detail-character">
              {katakana.find(c => c.character === selectedChar)?.character}
            </div>
            <div className="detail-info">
              <div className="detail-label">로마자</div>
              <div className="detail-value">
                {katakana.find(c => c.character === selectedChar)?.romaji}
              </div>
              <div className="detail-label">한글 발음</div>
              <div className="detail-value">
                {(() => {
                  const char = katakana.find(c => c.character === selectedChar)
